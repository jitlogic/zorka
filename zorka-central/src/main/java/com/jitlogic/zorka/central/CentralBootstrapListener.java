...
public class CentralBootstrapListener extends GuiceResteasyBootstrapServletContextListener {

    private final static Logger log = LoggerFactory.getLogger(CentralBootstrapListener.class);

    public void withInjector(final Injector injector) {
        injector.getInstance(ZicoService.class).start();
        Runtime.getRuntime().addShutdownHook(new Thread() {
            public void run() {
                System.out.println("Shutting down Central ...");
